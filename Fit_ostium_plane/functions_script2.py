...
def find_intersect(filename1, psource, ptarget):
    """
...
def get_halt(filename1, ori):
    ...
    final = np.add(ori, [0, 0, 40])
    origin = np.add(ori, [0, 0, 0.2])
    intersect, flag = find_intersect(filename1, origin, final)
    print(intersect)
    #if not intersect:
    #    origin = np.add(ori, [0, 0, -0.2])
    #    final = np.add(ori, [0, 0, -40])
    ...
    return h, intersect, h_comp

def get_hthetaalt(filename1, p0, h, cent, intersect):
    """
