...
for n,case in zip(range(len(list_caso_n4)),list_caso_n4):
    ...
    if case_num2 == 1:
        ...
        #compute the bending angle by maximum curvature of centerline
        nskippoints = round(start)
        # print "skipping ", nskippoints, " points from ", cl.GetNumberOfPoints()
        cl_ori = skippoints(rotated_c, int(nskippoints))
        clspacing = 0.4
        rotated_c = vmtkcenterlineresampling(cl_ori, clspacing)   


        centerline_geom = vmtkcenterlinegeometry(rotated_c,iterations=100, factor=0.5)
        ...
        point_cent = rotated_c.GetPoint(point_max)

        # print(CL_points_rotated[len(list_curvature)-1])
        ...
        # actor_pc = addpoint_show(point_cent, color=[0.0, 0.0, 0.0])
        # actor1 = addpoint_show(point1, color=[0.0, 0.0, 0.0])
        # actor3 = addpoint_show(point3, color=[1.0, 0.0, 0.0])
        # actorl1 = addline_show(point1, point_cent,color=[0.0, 0.0, 0.0])
        # actorl3 = addline_show(point_cent,point3, color=[0.0, 0.0, 0.0])

        bending = get_bending_2(point1,point_cent,point3)
        print('bending', bending)

        # renderer_h = vtk.vtkRenderer()
        # vtk_show_centreline(renderer_h, file,rotated_c, 1000, 1000, actor_pc,actor1,actor3, actorl1, actorl3)



        #save final ostium plane origin and normal, and centerline lengths
        print("Writing results...")
        raw_data = {'Patient_ID': [case_num2], 'ost_pos_x': [origin[0]], 'ost_pos_y': [origin[1]],'ost_pos_z': [origin[2]], 'ost_or_x': [normal[0]], 'ost_or_y': [normal[1]],'ost_or_z': [normal[2]], 'length_la': [length_la],'length_laa': [length_laa], 'bending':[bending]}
        df = pd.DataFrame(raw_data, columns=['Patient_ID', 'ost_pos_x', 'ost_pos_y','ost_pos_z', 'ost_or_x','ost_or_y','ost_or_z','length_la', 'length_laa', 'bending'])
